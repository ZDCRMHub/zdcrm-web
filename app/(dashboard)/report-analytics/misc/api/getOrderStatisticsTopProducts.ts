...
const getStats = async (options: FetchOptions = {}): Promise<APIResponse> => {
  ...
  const res = await APIAxios.get('/order/top-products/', { params });
  return res.data;
}

export const useGetTopProducts = (options: FetchOptions = {}) => {
  return useQuery({
    queryKey: ['orders-statistics-top-products', options],
