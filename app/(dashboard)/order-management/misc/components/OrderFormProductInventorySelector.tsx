...
import type { TProductInventoryItem, TProductVariation } from "@/app/(dashboard)/inventory/misc/types/products"
...
type enquiryItemType = z.infer<typeof enquiryItemSchema>
...
const OrderFormProductInventorySelector: React.FC<OrderFormProductInventorySelectorProps> = ({
    ...
    useEffect(() => {
        ...
        inventories?.forEach((inventory) => {
            const inv = options?.find((o) => o.id === inventory?.product_inventory_id) || null
            ...
            if (inv && inventory?.variations && inventory?.variations?.length > 0) {
                const currentVariations = inv.variations.filter((v) =>
                    inventory?.variations?.some(
                        (variation) => variation.product_inventory_variation_id === v.id,
                    ),
                )
            }
        })
    ...
    const handleSelect = (variation: TProductVariation) => {
        ...
        const isDefaultInventoryExist = inventories?.length === 1 && !inventories?.[0]?.variations?.length && !inventories[0]?.product_inventory_id && !inventories[0]?.stock_inventory_id
        ...
        const inventoryIndex = updatedInventories?.findIndex((inv) => inv?.product_inventory_id === (selectedOption?.id || 0))
        ...
        if (inventoryIndex === -1) {
            ...
            updatedInventories?.push({
                ...
                variations: [
                    {
                        stock_variation_id: undefined,
                        product_inventory_variation_id: variation.id,
                        quantity: 1,
                    },
                ],
            })
            ...
            const variationIndex = existingInventory.variations?.findIndex(
                (v) => v.product_inventory_variation_id === variation.id,
            )
    ...
    const getQuantityForVariation = (variationId: number) => {
        ...
        for (const inventory of inventories) {
            const v = inventory?.variations?.find((v) => v.product_inventory_variation_id === variationId)
    ...
    const updateQuantityForVariation = (variationId: number, quantity: number) => {
        ...
        for (let i = 0; i < updatedInventories?.length; i++) {
            ...
            const idx = inventory?.variations?.findIndex((v) => v.product_inventory_variation_id === variationId)
    ...
    const getSelectedVariations = () => {
        const selectedVariations: TProductVariation[] = []
        inventories?.forEach((inventory) => {
            const inv = options?.find((o) => o.id === inventory?.product_inventory_id)
            inventory?.variations?.forEach((variation) => {
                const v = inv?.variations.find((v) => v.id === variation.product_inventory_variation_id)
                                    ...
                                    : options?.length === 0
                                        ...
                                        : (inventories?.length ?? 0) > 0
                                            ? inventories?.length === 1
                                                ? `${options?.find((o) => o.id === inventories?.[0]?.product_inventory_id)?.name} (${inventories?.[0]?.variations?.length} variations)`
                        ...
                        <div className="space-y-3">
                            <Label className="text-sm font-medium">Selected Variations & Quantities</Label>
                            {
                                inventories?.map((inventory) => {
                                    const product = options?.find((o) => o.id === inventory?.product_inventory_id)
                                    return inventory?.variations?.map((variation) => {
                                        const v = product?.variations.find((v) => v.id === variation.product_inventory_variation_id)
                                        return (
                                            <div
                                                key={variation.product_inventory_variation_id}
                                            ...
                                            >
                                                ...
                                                <div className="flex items-center gap-2">
                                                    <Label htmlFor={`quantity-${variation.product_inventory_variation_id}`} className="text-xs">
                                                        Qty:
                                                    </Label>
                                                    <input
                                                        id={`quantity-${variation.product_inventory_variation_id}`}
                                                        type="number"
                                                        min={1}
                                                        value={getQuantityForVariation(variation.product_inventory_variation_id || 1)}
                                                        onChange={(e) =>
                                                            updateQuantityForVariation((variation.product_inventory_variation_id || 1), Number(e.target.value))
                                                        }
