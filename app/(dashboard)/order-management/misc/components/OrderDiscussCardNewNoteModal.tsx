...
import { useAddNoteToOrderDiscussion } from '../api'
...
const OrderDiscussCardNewNoteModal: React.FC<ModalProps> = ({ isModalOpen, closeModal, order_id, setModalOpen }) => {
    const [newNote, setNewNote] = React.useState('')
    const handleNoteChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
        setNewNote(e.target.value)
    }
    const { mutate, isPending } = useAddNoteToOrderDiscussion({ id: order_id })
    const handleSubmit = () => {
        if (!newNote) return;
        if (newNote.length < 10) {
            toast.error('Note should be atleast 10 characters long')
        }
        mutate({ id: order_id, message: newNote }, {
            onSuccess: () => {
                closeModal()
            }
        })
...
export default OrderDiscussCardNewNoteModal