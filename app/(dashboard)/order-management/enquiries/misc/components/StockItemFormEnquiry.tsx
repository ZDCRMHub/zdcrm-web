      ...
      <div className="grid grid-cols-2 xl:grid-cols-3 gap-10 mb-8">
        <StockItemSelector
          setInventoryId={(inventoryId) => {
            setValue(`items.${index}.inventories.${index}.stock_inventory_id`, inventoryId);
          }}
          setVariations={(variations) => {
            setValue(`items.${index}.inventories.${index}.variations`, variations);
          }}
...

        {
          categoryName === 'Cake' && (
            <>
              ...
              <Controller
                name={`items.${index}.inventories.${index}.properties.layers`}
                control={control}
                render={({ field }) => (
                  <SelectSingleCombo
                    options={
                    ...
                    {...field}
                    hasError={!!errors.items?.[index]?.inventories?.[index]?.properties?.layers}
                    errorMessage={errors.items?.[index]?.inventories?.[index]?.properties?.layers?.message as string}
              ...
              <Controller
                ...
                render={({ field }) => (
                  <SelectSingleCombo
                    ...
                    {...field}
                    hasError={!!errors.items?.[index]?.inventories?.[index]?.properties?.toppings}
                    errorMessage={errors.items?.[index]?.inventories?.[index]?.properties?.toppings?.message as string}
              ...
              <Controller
                name={`items.${index}.inventories.${index}.properties.whipped_cream_upgrade`}
                control={control}
                render={({ field }) => (
