...
import { TFormItemSelectionOption } from './EnquiryFormItemsSection';
...
interface StockItemFormEnquiryProps {
  ...
  onChange: (selectedItems: TFormItemSelectionOption[]) => void;
  label?: string;
  placeholder?: string;
  disabled?: boolean;
...
const StockItemFormEnquiry: React.FC<StockItemFormEnquiryProps> = ({
  ...
  const [selectedItems, setSelectedItems] = useState<TFormItemSelectionOption[]>([]);
  ...
  const getButtonText = () => {
    ...
    if (selectedItems.length === 1) return `${selectedItems[0].name} - ${selectedItems[0].variation}`;
  ...
  return (
    <div>
      <Popover open={open} onOpenChange={setOpen}>
        ...
        <PopoverContent className={cn("w-full overflow-hidden min-w-[1000px] max-w-[1400px]", triggerRef?.current && `min-w-max`)} >
          ...
          <div className="grid grid-cols-2 xl:grid-cols-3 gap-4 min-w-max h-max min-h-[16rem] max-h-[30rem] overflow-scroll overflow-y-auto">
            {filteredOptions?.map((option) => (
              <button
                className={cn("text-xs text-[#194A7A] relative flex !flex-col select-none items-center rounded-2xl p-2 outline-none aria-selected:bg-blue-100/70 aria-selected:text-accent-foreground data-[disabled]:pointer-events-none data-[disabled]:opacity-50",
                  "text-sm min-w-[150px] min-h-[150px] w-max", selectedItems.some(i => i.id === option.id) &&  "bg-custom-blue !text-white"
                )}
                key={option.id}
              ...
              >
                ...
                <p className="text-[0.75rem] pt-1.5 max-w-[130px]">
