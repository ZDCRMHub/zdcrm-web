...
export const generateMockOrders = (count: number) => {
    ...
    return Array.from({ length: count }, () => ({
        ...
        orderId: `${faker.helpers.arrayElement(['ZD', 'PF'])}/${faker.string.alpha({ length: 2, casing: 'upper' })}${faker.string.numeric(4)}`,
        action,
...
export const OrderCard = ({ order, hideOtherInfo }: { order: TDiscusssion, hideOtherInfo?: boolean }) => {
    const {
        state: isModalOpen,
        setTrue: openModal,
        setFalse: closeModal,
        ...
        <>
            <Accordion type="single" collapsible className="w-full max-w-[1440px] px-8" defaultValue='today'>
                ...
                <AccordionItem value="tomorrow">
                    <AccordionTrigger>Tomorrow, {format(new Date(new Date().setDate(new Date().getDate() + 1)), 'dd MMMM yyyy')}</AccordionTrigger>
                ...
                <AccordionItem value="within72Hours">
                    <AccordionTrigger>{format(new Date(new Date().setDate(new Date().getDate() + 1)), 'qo dd MMMM yyyy')}</AccordionTrigger>
                    <AccordionContent className='px-4'>
