...
const EnquiryDiscussCard = ({ order, hideOtherInfo, isExpanded = false }: { order: TDiscusssion, hideOtherInfo?: boolean; isExpanded?: boolean; }) => {
    const {
        state: isModalOpen,
        setTrue: openModal,
        setFalse: closeModal,
        ...
        <Card className="mb-4 border border-[#194A7A] bg-[#f4f6f3]">
            <CardContent className='flex items-start pt-4'>
                ...
                <section className='flex-[2]'>
                    <CardTitle className='text-[#194A7A] font-semibold text-[1.35rem]'>{order.customerName} just {order.action} {order.type !== 'enquiry' && order.orderId}</CardTitle>
