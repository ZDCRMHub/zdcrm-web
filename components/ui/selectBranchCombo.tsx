...
import { SearchIcon, ArrowLeft, CheckIcon } from 'lucide-react'
...
const SelectBranchCombo = ({
  ...
  // lists come from hooks (don't store lists in local state)
    ...
    <div className={cn('inputdiv', withIcon && 'withicon', containerClass)}>
      <Popover open={isOpen} onOpenChange={setOpen}>
        <div className="flex flex-col gap-2">
          <Label className="text-sm text-[#0F172B] font-poppins font-medium" htmlFor={name || 'branch-select'}>
            Branch
          ...
          <PopoverTrigger asChild>
            ...
            >
              <span className={cn('!overflow-hidden text-sm w-full font-normal', (value && branchesList && branchesList.length) ? '' : '!text-[#A4A4A4]')}>
                ...
                <>
                  ...
                  {(!(isLoadingBranches) && branchesList && branchesList.length > 0) ? (
                    (filteredBranches || branchesList).map((br) => (
                      ...
                      >
                        <CheckIcon className={cn('mr-2 h-4 w-4', br.id == String(value) ? 'opacity-100' : 'opacity-0')} />
                        {br.name}
                      </button>
                    ))
                  ) : (
